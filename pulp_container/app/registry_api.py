    ...
    def handle_safe_method(self, request, path, pk):
        ...
        try:
            ...
            if repository.PUSH_ENABLED:
                ...
                # TODO: verify if the blobs are already available because of the dispatched task
                ...
                #     redirects = FileStorageRedirects(distribution, path, request)
...
class Manifests(RedirectsMixin, ContainerRegistryApiMixin, ViewSet):
        ...
        if pk[:7] != "sha256:":
            try:
                ...
                if distribution.remote:
                    ...
                    repository = distribution.repository.cast()
                    manifest, response = self.get_manifest_from_local_storage(remote, pk)
                    # if not manifest:
                    #    artifact = self.pull_manifest_from_remote(remote, request, pk)
                    # self.handle_pending_manifest(manifest, repository, pk)
                    if not manifest:
                        if response.status_code == 200:
                            # TODO: consider calling redirects.redirect_to_content_app
                            redirects = FileStorageRedirects(distribution, path, request)
                            return redirects.issue_tag_redirect(T(pk))
                        else:
            ...
            try:
                ...
                if repository.PUSH_ENABLED:
                    ...
                    if distribution.remote:
                        ...
                        # TODO: relations between manifests and manifest lists will be handled
                        #   through the task dispatched from content-app
                        ...
                        redirects = FileStorageRedirects(distribution, path, request)
                        return redirects.issue_manifest_redirect(M(pk))
                    else:
    ...
    def get_manifest_from_local_storage(self, remote, pk):
        ...
        try:
            ...
            # TODO: handle a temporary outage by returning a manifest that was downloaded before;
            #   if the manifest does not exist locally, then raise ManifestNotFound
            raise ManifestNotFound(reference=pk)

        digest = response.headers.get("docker-content-digest")
        return models.Manifest.objects.filter(digest=digest).first(), response
